...
QPC_final <- QPC_final %>%
  mutate(Country_1 = if_else(grepl("654|366|626|724|668|243|692|564|688|646|746|676|213|614", Country.Code), 1, 0))


QPC_final <- QPC_final %>%
  mutate(Country_2 = if_else(grepl("718|714|624|738|911|963|638|915|942|439|748|343|716|648|", Country.Code), 1, 0))
...
tabla6$Program_spam <- tabla6$Initial.End.Date - tabla6$Approval.Date
...
tabla6$QPC_spam <- tabla6$QPC_Test_Date_ - tabla6$Approval.Date_
tabla6$QPC_date_spam <- as.numeric(tabla6$QPC_spam)

tabla6 <- tabla6[, -which(names(tabla6) == "QPC_Test_Date_")]
tabla6 <- tabla6[, -which(names(tabla6) == "Approval.Date_")]
tabla6 <- tabla6[, -which(names(tabla6) == "QPC_spam")]
...
tabla6 <- tabla6 %>% select(-Program_spam)

tabla6 <- tabla6 %>%
  relocate(Joint_revision, .before = 6)

tabla6 <- tabla6 %>%
  relocate(QPC_date_spam, .before = 5)
...
tabla6$QPC_Amount_spam <- tabla6$QPC_Actual_Amount_real - tabla6$QPC_Original_Amount_real
...
tabla6 <- tabla6 %>%
  mutate(shock_II = ifelse(Year %in% c(2009, 2010, 2022, 2023, 2024), 1, 0))
...
write_xlsx(tabla6, "C:/Users/HP/Desktop/Tesis UDESA/tabla_para_modelos12.xlsx")
